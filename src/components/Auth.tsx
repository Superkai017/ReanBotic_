...
import { Bot, Mail, Lock, User, Check, ArrowRight, Sparkles, Building, GraduationCap, Laptop, Cpu, Award, Sun, Moon } from 'lucide-react';
...
export default function Auth({ onSuccess, darkMode, onToggleDarkMode }: AuthProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-6 font-sans relative overflow-hidden transition-colors duration-300">
      {/* Background Decorative Gradients */}
      <div className="absolute top-1/4 left-1/4 w-[400px] h-[400px] bg-blue-200/40 dark:bg-blue-900/10 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-1/4 right-1/4 w-[400px] h-[400px] bg-emerald-200/40 dark:bg-emerald-900/10 rounded-full blur-3xl pointer-events-none" />

      {/* Theme toggle */}
      <button
        onClick={onToggleDarkMode}
        className="absolute top-6 right-6 z-20 p-2.5 text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm transition-colors cursor-pointer"
      ...
      <AnimatePresence mode="wait">
        {step === 'form' ? (
          <motion.div
            ...
            className="w-full max-w-md bg-white border border-slate-200 dark:bg-slate-900/50 dark:border-slate-900 rounded-3xl p-8 backdrop-blur-md shadow-2xl relative z-10"
          >
            {/* Header branding */}
            <div className="flex flex-col items-center text-center mb-8">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1.5">Join Reanbotic and learn gamified robotics today.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-300 text-slate-900 placeholder-slate-400 dark:bg-slate-950 dark:border-slate-800 dark:text-white dark:placeholder-slate-600 rounded-2xl focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-sm transition"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Education Level</label>
              ...
              <div className="flex items-start gap-3">
                <input
                  ...
                  className="mt-1 w-4 h-4 rounded text-blue-600 border-slate-300 dark:border-slate-800 dark:bg-slate-950 focus:ring-blue-500"
                />
                <label htmlFor="agree-checkbox" className="text-xs text-slate-600 dark:text-slate-400 leading-normal">
              ...
              <button
                ...
                className="w-full py-4 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-300 disabled:text-slate-500 dark:disabled:bg-slate-800 dark:disabled:text-slate-600 text-white font-bold rounded-2xl shadow-xl shadow-blue-600/20 transition-all flex items-center justify-center gap-2 group active:scale-95 cursor-pointer"
              >
                Create Account
          ...
          <motion.div
            ...
            className="w-full max-w-2xl bg-white border border-slate-200 dark:bg-slate-900/50 dark:border-slate-900 rounded-3xl p-8 backdrop-blur-md shadow-2xl relative z-10 text-center"
          >
            <div className="p-4 bg-emerald-100 border border-emerald-200 text-emerald-600 dark:bg-emerald-500/10 dark:border-emerald-500/20 dark:text-emerald-400 rounded-full w-fit mx-auto mb-4">
              <Award className="w-8 h-8" />
            </div>

            <h2 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Welcome, {formData.name}! 🎉</h2>
            <p className="text-slate-500 dark:text-slate-400 mt-2 text-base">Your account has been configured. Now, choose your robotics path:</p>
            ...
            <div className="grid sm:grid-cols-3 gap-4 mt-8">
              {[
                ...
                <button
                  ...
                  className={`p-5 border rounded-2xl flex flex-col items-center text-center gap-3 cursor-pointer transition-all ${selectedSpec === p.id ? 'border-blue-500 bg-blue-50 dark:bg-blue-950/20 shadow-lg shadow-blue-500/5' : 'border-slate-200 bg-slate-50 hover:border-slate-300 dark:border-slate-800 dark:bg-slate-950 dark:hover:border-slate-700'}`}
                >
                  <div className={`p-3 rounded-xl ${selectedSpec === p.id ? 'bg-blue-100 text-blue-600 dark:bg-blue-500/10 dark:text-blue-400' : 'bg-slate-100 text-slate-500 dark:bg-slate-900 dark:text-slate-500'}`}>
